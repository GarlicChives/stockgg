...
const ALLOWED: Set<string> = new Set([
  ...
  // Q19 — 某 ETF 最新交易日 holdings + 對前一交易日 diff(供 ETF 頁 tab 內容)。
  // FULL OUTER JOIN 讓 exit(today 沒、yesterday 有)也能 surface 算 action chip。
  "with last_two as (select distinct holding_date from active_etf_holdings where etf_code = $1 order by holding_date desc limit 2), latest as (select max(holding_date) as d from last_two), prev as (select min(holding_date) as d from last_two) select coalesce(t.ticker, y.ticker) as ticker, coalesce(t.name, y.name) as name, t.lots, t.weight_pct, t.market_value_ntd, t.market, t.is_cash, y.lots as prev_lots, coalesce(t.lots, 0) - coalesce(y.lots, 0) as lots_chg, case when t.lots is null or t.lots = 0 then 'exit' when y.lots is null or y.lots = 0 then 'new' when t.lots > y.lots then 'add' when t.lots < y.lots then 'reduce' else 'hold' end as action from (select * from active_etf_holdings where etf_code = $1 and holding_date = (select d from latest)) t full outer join (select * from active_etf_holdings where etf_code = $1 and holding_date = (select d from prev)) y on t.ticker = y.ticker order by t.weight_pct desc nulls last",

  // Q20 — 某個股被哪些主動 ETF 持有 + diff + 佔流通比重(供個股 modal)。
  // pct_of_float = lots × 1000 / stock_meta.shares_outstanding × 100(lots 是
  // 「張」,千股為單位)。WHERE t.lots IS NOT NULL OR y.lots IS NOT NULL 過濾
  // 從沒持有此股的 ETF。ORDER BY aum desc 對齊 ETF 頁 tab 順序。
  "with last_two as (select etf_code, holding_date, row_number() over (partition by etf_code order by holding_date desc) as rn from active_etf_holdings where ticker = $1), latest_per as (select etf_code, holding_date from last_two where rn = 1), prev_per as (select etf_code, holding_date from last_two where rn = 2) select m.etf_code, m.etf_name, m.short_name, m.issuer, m.aum_ntd, t.holding_date, t.lots, t.weight_pct, t.market_value_ntd, y.lots as prev_lots, coalesce(t.lots, 0) - coalesce(y.lots, 0) as lots_chg, round(t.lots * 1000.0 / nullif(sm.shares_outstanding, 0) * 100, 3) as pct_of_float, case when t.lots is null or t.lots = 0 then 'exit' when y.lots is null or y.lots = 0 then 'new' when t.lots > y.lots then 'add' when t.lots < y.lots then 'reduce' else 'hold' end as action from active_etf_meta m left join active_etf_holdings t on t.etf_code = m.etf_code and (t.etf_code, t.holding_date) in (select etf_code, holding_date from latest_per) and t.ticker = $1 left join active_etf_holdings y on y.etf_code = m.etf_code and (y.etf_code, y.holding_date) in (select etf_code, holding_date from prev_per) and y.ticker = $1 left join stock_meta sm on sm.ticker = $1 where t.lots is not null or y.lots is not null order by m.aum_ntd desc nulls last",
])
