...
# 策略清單「fallback 序」(僅 Q47 查詢失敗時用)。正常情況哪些 slug 會出現完全由 DB
# strategy_backtest_public 現有列決定(ingest 寫 DB → 自動上架,本檔零改動),tab 顯示序
# 則依各策略 payload.metrics.sharpe 由高到低(見 STRAT_ORDER 計算)。此表只在連 slug 清單
# 都查不到時當保險墊;新增策略不需要編輯這裡。
...
def _clean_strat_name(name: str) -> str:
    return _STRAT_NAME_SUFFIX_RE.sub("", (name or "")).strip()

_NEXT_COND = {"vol": "出量", "nh": "新高", "growth": "成長", "chip": "籌碼", "pot": "潛力"}


def _theme_chips_html(tk, theme_map: dict | None, hot_themes: set | None) -> str:
    """ticker → 所屬焦點題材 chip 串(無外層容器)。今日焦點題材(hot_themes)以 .hot accent
    高亮。總儀表板買進列/共識 pill 與各策略 tab 明日買進卡共用。無題材回空字串。"""
    prefs = (theme_map or {}).get(str(tk)) or []
    if not prefs:
        return ''
    hot = hot_themes or set()
    esc = html_lib.escape
    return "".join(f'<span class="dash-th{" hot" if p in hot else ""}">{esc(p)}</span>'
                   for p in prefs)
...
def _build_trade_next_html(next_rows: list[dict] | None,
    ...
    for r in rows:
        ...
        # 再進場買單(ingest 0a2a622:reentry/reentry_kind)— 與雷達 conds 徽章區隔(琥珀)
        _re_chip = ''
        if r.get("reentry"):
            _rek = r.get("reentry_kind")
            _re_chip = ('<span class="sim-next-re">再進場·'
                        f'{html_lib.escape(str(_rek))}</span>' if _rek
                        else '<span class="sim-next-re">再進場</span>')
        hot = tk in seeds
        ...
        if _bs:
            ...
            _wr = _bs.get("wr")
            _best = _bs.get("best")
            _tot_cls = ('up' if isinstance(_tot, (int, float)) and _tot > 0
                        else ('down' if isinstance(_tot, (int, float)) and _tot < 0 else 'neutral'))
            _tot_s = (f"{'+' if _tot > 0 else ''}{_tot:.1f}%" if isinstance(_tot, (int, float)) else '—')
            _best_s = (f"+{_best:.1f}%" if isinstance(_best, (int, float)) else '—')
            _bt_meta = (f"{_n if _n is not None else '—'}筆·勝"
                        f"{_wr if _wr is not None else '—'}%·最佳{_best_s}")
        ...
        (re_cards if r.get("reentry") else cards).append(
            ...
            + (f'<div class="sim-next-conds">{_re_chip}{chips}</div>' if (_re_chip or chips) else '')
            + (lambda th: f'<div class="sim-next-themes">{th}</div>' if th else '')(
                _theme_chips_html(tk, theme_map, hot_themes))   # 所屬題材列
            + bt_html
            + '</div>'
        )
    return (
        ...
        + '</div>'
        # gap 策略專屬(2026-06-20):卡片顯示的 % = 當日「收盤漲幅」,與規則的「開盤跳空
        # ≤ +9%」是不同的量;不加會被誤讀為自相矛盾(收盤可達漲停)。僅此策略需要。
        + ('<p class="sim-next-note sim-next-gapclar">⚠ 卡片上的百分比為當日<b>收盤漲幅</b>'
           '(收盤相對昨收);進場規則的「跳空 ≤ +9%」指的是<b>開盤相對昨收的隔夜跳空</b>,'
           '兩者為不同數值 —— 一檔可「開盤跳空 +5%、盤中續攻收盤漲停 +10%」,故收盤漲幅'
           '可達漲停、與規則並不矛盾。</p>' if slug == "gap" else '')
    ...
    **只有 pullback** 才 fallback 讀靜態 data/pullback_public.json(離線 / 過渡保險);
    ...
    if d is None:
        ...
        try:
            ...
            return ""
    esc = html_lib.escape
    _sname = esc(_clean_strat_name(d.get("name")) or STRAT_NAME)   # 策略名動態讀 payload(去「(深度…)」尾綴;勿硬寫舊值)
    m = d.get("metrics") or {}
    ...
    rows_html = (
        f'<tr><td class="sim-m-name"><i style="background:#60a5fa"></i>{_sname}</td>'
    ...
    # gap 策略專屬釐清(2026-06-20):規則的「跳空」= 開盤相對昨收的隔夜 gap(≤ +9%);
    # 標的卡顯示的 % = 當日「收盤漲幅」(收盤相對昨收),兩者為不同數值,收盤漲幅可達漲停。
    _pb_clar = ('<p class="sim-bt-pb-clar">⚠ 規則中的「跳空」指<b>開盤相對昨收的隔夜跳空</b>'
                '(≤ +9%);上方明日標的卡顯示的百分比為當日<b>收盤漲幅</b>(收盤相對昨收)——'
                '兩者為不同數值,故收盤漲幅可達漲停而不違反「跳空 ≤ +9%」。</p>'
                if slug == "gap" else "")
    pb_block = (f'<div class="sim-bt-playbook"><div class="sim-bt-pb-h">策略說明(精簡版)</div>'
                f'<ol>{pb_html}</ol>{_pb_clar}</div>') if pb_html else ""
...
def _build_dashboard_html(dash: dict | None,
    ...
    def _theme_html(tk: str) -> str:
        ...
        chips = _theme_chips_html(tk, themes, hot_th)
        return f'<span class="dash-th-row">{chips}</span>' if chips else ''
    strategies = dash.get("strategies") or []
    benchmarks = dash.get("benchmarks") or {}
    _aod = str(dash.get("as_of_date") or "")[:10]
    # 共識三類(2026-06-20 user)全由本檔從各策略 watchlist/sells 推導(不再吃 ingest
    # consensus_picks),才能同源切出買進/賣出/分歧三欄。consensus_tk(grid 🤝)= 純買進共識。
    consensus_tk: set[str] = set()

    def _sname(obj):
        return esc(_clean_strat_name(obj.get("name"))
                   or _STRAT_FALLBACK_NAME.get(obj.get("slug"), obj.get("slug") or ""))
    ...
    # 共識題材(2026-06-21 user):≥2 不同策略各自買進「同一焦點題材」的標的(標的可不同)→
    # 題材層級共識(例:記憶體模組)。同一組標的常對應多個近義題材前綴(DRAM/HBM/記憶體模組…),
    # 故以「標的集合」群組去重、每組留一個代表(優先今日焦點題材,其餘列 title)避免近義刷屏。
    _slug_obj = {s.get("slug"): s for s in strategies}
    _th_strats: dict[str, set] = {}     # 題材前綴 → {slug}
    _th_tks: dict[str, dict] = {}       # 題材前綴 → {ticker: name}(保插入序)
    for s in strategies:
        _slug = s.get("slug")
        for row in (s.get("watchlist") or []):
            tk = str(row.get("ticker") or "")
            if not tk:
                continue
            for p in (themes.get(tk) or []):
                _th_strats.setdefault(p, set()).add(_slug)
                _th_tks.setdefault(p, {}).setdefault(tk, str(row.get("name") or tk))
    _groups: dict[frozenset, dict] = {}
    for p in _th_strats:
        if len(_th_strats[p]) < 2:
            continue
        key = frozenset(_th_tks[p])
        g = _groups.setdefault(key, {"prefs": [], "strats": set(), "tks": _th_tks[p]})
        g["prefs"].append(p)
        g["strats"] |= _th_strats[p]
    consensus_themes = []
    for g in _groups.values():
        rep = next((p for p in g["prefs"] if p in hot_th), g["prefs"][0])
        consensus_themes.append({
            "theme": rep,
            "alts": [p for p in g["prefs"] if p != rep],
            "strats": sorted(g["strats"]),
            "tks": g["tks"]})
    # 排序:今日焦點題材在前 → 策略數多 → 標的數多 → 名稱
    consensus_themes.sort(key=lambda c: (c["theme"] not in hot_th, -len(c["strats"]),
                                         -len(c["tks"]), c["theme"]))

    def _cth_row(c):
        rep = c["theme"]
        title = ("亦屬:" + "、".join(c["alts"])) if c["alts"] else ""
        strat_chips = "".join(
            f'<span class="dash-cons-st">{_sname(_slug_obj.get(sl, {"slug": sl}))}</span>'
            for sl in c["strats"])
        tk_pills = "".join(
            f'<button type="button" class="dash-cth-tk" {_click(tk, nm)}>'
            f'<b>{esc(tk)}</b> {esc(nm)}{_hot_badge(tk)}</button>'
            for tk, nm in c["tks"].items())
        return (
            f'<div class="dash-cth-row"{f" title={json.dumps(title)}" if title else ""}>'
            f'<span class="dash-cth-name{" hot" if rep in hot_th else ""}">{esc(rep)}</span>'
            f'<span class="dash-cth-strats">{strat_chips}</span>'
            f'<span class="dash-cth-tks">{tk_pills}</span>'
            '</div>')

    # ── 區塊 1:各策略買進共識(共識題材 + 共識個股;賣出/分歧已移除)──
    sec_consensus = ""
    if consensus_themes or buy_consensus:
        _parts = []
        if consensus_themes:
            _rows = "".join(_cth_row(c) for c in consensus_themes)
            _parts.append(
                '<div class="dash-cth"><div class="dash-cth-hd">🎯 共識題材'
                '<span class="dash-cth-sub">≥2 策略各自買進同題材標的(標的可不同)· '
                '今日焦點題材以色塊標示</span></div>'
                f'<div class="dash-cth-list">{_rows}</div></div>')
        if buy_consensus:
            _pills = "".join(_cons_pill(p) for p in buy_consensus)
            _parts.append(
                '<div class="dash-cons-block"><div class="dash-cth-hd">🔁 共識個股'
                '<span class="dash-cth-sub">≥2 策略買進同一檔</span></div>'
    ...
    sec_buy = (
        ...
        + '<div class="dash-strat-grid">' + "".join(buy_cards) + '</div>'
    ...
    # ── 段 ③:共識買回測績效(2026-06-20 ingest a5a2336;🅰️ 無限資金 / 🅱️ 1000 萬資金)──
    ...
    _CBT = [("consensus_unlimited", "🅰️ 無限資金",
            ...
            ("consensus_300m", "🅱️ 1000 萬資金",
             "有限資金引擎:1000 萬本金、無融資無停泊、受實際下單與部位上限約束(貼近真實可執行績效)。")]
...
def build_trade_sim_page(strat_data: dict | None = None,
    ...
    順序由 `strat_order` 決定(夏普由高到低)。`strat_data` = {slug:{payload, sim_next,
    bt_stats, bt_trades_n}}。無 dashboard 時第一個 slug 預設 active。回測為無限資金、含成本
    真實回測;payload 缺則該 slug 不渲回測區塊(pullback 另有靜態檔 fallback)。"""
    esc = html_lib.escape
    strat_data = strat_data or {}
    ...
    def _name(slug):
        ...
        return _clean_strat_name(pl.get("name")) or _STRAT_FALLBACK_NAME.get(slug, slug)

    btns, panes = [], []
    # 總儀表板(若有)= 第一個 sub-tab、預設 active;沿用 .strat-tab-btn / .strat-pane
    # 視覺語彙與 showStrategyTab 切換邏輯(JS 端對 dashboard 的 chart/逐筆 lazy-init 皆 no-op)。
    if dash_html:
        btns.append('<button class="strat-tab-btn active" type="button" data-strat="dashboard" '
                    'onclick="showStrategyTab(\'dashboard\')">📊 總儀表板</button>')
        panes.append('<div class="strat-pane active" id="strat-dashboard" '
                     'data-slug="dashboard">' + dash_html + '</div>')
    for i, slug in enumerate(order):
        sd = strat_data[slug]
        active = " active" if (i == 0 and not dash_html) else ""
        btns.append(
...
async def generate():
    ...
    # Q40 / Q41(trade_sim_nav / trade_sim_trades)即時 300萬 paper-trading 版
    ...
    # 即自動上架,本檔不需改)。"dashboard" 是『所有策略的聚合列』、非單一策略 → 從策略 tab
    # 清單剔除(單獨渲染為「總儀表板」第一個 sub-tab)。tab 顯示順序 = 各策略
    # payload.metrics.sharpe 由高到低(冠軍在最左;見迴圈後 STRAT_ORDER 排序)。查詢失敗才
    # fallback 偏好序。
    try:
    ...
    #   consensus_300m       → 總儀表板內「共識買回測 🅱️ 1000 萬資金」(slug 名沿用 300m;ingest B 基礎 v2 已改 1000 萬)
    ...
    def _ct_compact(t):
        return [
            ...
            t.get("entry_date"), _jc(t.get("entry_price"), 2),
            t.get("exit_date"), _jc(t.get("exit_price"), 2),
            _jc(t.get("pnl_pct"), 2),
    ...
    # 策略 tab 顯示序:payload.metrics.sharpe 由高到低(冠軍最左)。缺 sharpe 者排末尾、
    # 字母序 tie-break。預設 active tab = 總儀表板(有的話),否則第一個策略。
    def _sharpe_of(s):
        m = ((strat_data.get(s) or {}).get("payload") or {}).get("metrics") or {}
        v = m.get("sharpe")
        return v if isinstance(v, (int, float)) else float("-inf")
    STRAT_ORDER = sorted(_strat_slugs, key=lambda s: (-_sharpe_of(s), s))
    print(f"  策略 tab 顯示序(夏普降序): {STRAT_ORDER}")

    # 總儀表板(ingest build_dashboard;slug='dashboard')— 純聚合所有策略,只讀 payload
    # (無 sim_next / 逐筆)。Q44 已 slug 參數化(allowlist 涵蓋),不需新增 query。缺(舊
    # 資料)→ None → build_trade_sim_page 不渲該 tab。
    dashboard_payload = None
    if _dash_in_db:
        try:
            _dr = await conn.fetchrow(
                "select payload from strategy_backtest_public where slug = $1", "dashboard")
            if _dr and _dr.get("payload"):
                _dp = _dr["payload"]
                dashboard_payload = json.loads(_dp) if isinstance(_dp, str) else dict(_dp)
                print(f"  總儀表板 (dashboard): {len(dashboard_payload.get('strategies') or [])} 策略 / "
                      f"top_picks {len(dashboard_payload.get('top_picks') or [])} / "
                      f"共識 {len(dashboard_payload.get('consensus_picks') or [])}")
            else:
                print("  ⚠ dashboard payload 為空")
        except Exception as exc:
            print(f"  ⚠ 總儀表板 dashboard payload fetch 失敗: {exc}")
