...
- `docs/kline.json` — 個股 modal 日 K 線 lazy fetch payload(P2 2026-05-25 v2)。**單一檔案含全部 ticker**:`{"b": build_stamp_iso, "k": {"<ticker>": [[d,o,h,l,c,v],...], ...}}`。~8MB / gzip ~2MB,450 ticker。`docs/app.js` 的 `_loadKlineAll` 第一次任意 ticker 點 modal 才 lazy fetch — **必須用 `fetch('kline.json', { cache: 'no-cache' })`,不可加 `?_=Date.now()` 隨機 query**(每次 URL 不同會讓 Cloudflare 邊緣節點每次都 cache miss → 實地 query origin Worker → 撞 Workers Static Assets manifest sync 延遲導致 404,跟 history.json 模式不一致);固定 URL + revalidate 才能命中邊緣 cache 穩定快速,deploy 後 Cloudflare 會自動 invalidate。解析後存全域 dict,後續 `_fetchKline(tk)` 從 in-memory 拿。**v1 per-ticker `docs/kline/<tk>.json` 架構廢棄**:450 個獨立 manifest entry 在 Cloudflare edge sync 延遲嚴重(實測 deploy ✨ uploaded 450 全 OK,但 user 4 秒內 retry x3 仍 404)。同時保留 per-ticker 檔一段時間作 fallback。`.gitignore` 排除 docs/kline/(每次 regen 重寫),wrangler v4 部署 Static Assets 不 respect `.gitignore`,kline/ 仍會被上傳。相當於用「per-regen 寫靜態檔」取代「client-side db-proxy fetch + 暴露 anon key」。Q13 維持 400 天(730 天會爆 db-proxy 6MB response 上限),所以 kline 也只有 ~400 天,UI 時間粒度 chip 上限 1Y。**JSON 結構含 `b: build_stamp`** 讓每次 regen 內容 hash 必變,wrangler 強制重傳(避過去 deploy abort 殘留 cache 對不上 Cloudflare KV 狀態的 bug)。(2026-05-25 連環踩過 .gitignore / .assetsignore / wrangler manifest cache / per-ticker entry sync / cache-bust query 反害 五層誤判)
