...
- `supabase/functions/db-proxy-public/index.ts` — Edge Function 含 SQL allowlist(目前 **36 條**):
  ...
  - **Q38** 產業地圖(🗺️ 產業地圖 tab,2026-06-07;ingest 週日 09:30 cron 爬 statementdog 焦點產業階層寫 `industry_focus_map`):`SELECT focus_tag, focus_name, axis, axis_kind, axis_order, sub_industry, sub_order, description, ticker, stock_name, market, rating, rating_rank FROM industry_focus_map ORDER BY focus_name, axis_order, sub_order, rating_rank DESC, ticker`。~1095 列編輯型慢變,整表一次撈。**呈現 = 供應鏈蜘蛛網有向圖(2026-06-07 改版;原 flat 清單廢、原「交集股連線」也廢)**:`build_industry_map_page(rows, stocks_info, supply_edges)` —— 節點=50 焦點產業,**邊=焦點間供應鏈有向邊(Q39,from=上游→to=下游,箭頭指下游)**,非交集股。**節點發亮 = 該焦點今日成分股的成交值加權平均漲跌幅**(用 ticker join 記憶體 `stocks_info`,紅=今日強/綠=弱/灰空心=今日無成交資料;加權漲幅 ≥ `HOT_THRESHOLD=2.0` 且 cov≥0.2 → 脈動光暈)。`app.js`:`_initIndmapGraph`(手刻 Fruchterman-Reingold 力導向 + radius 解重疊 + 原生 SVG glow + **有向邊箭頭/relation hover tooltip**;**滿版 = viewBox 寬度跟容器寬高比走填滿、無滾輪縮放/拖曳**)、`_imHeatColor`、`_imLayout`。`.im-page` 用 `.wrap` 90% 寬(無 1080 cap)。payload `window.IIA_INDMAP_GRAPH={nodes,edges:[from,to,strength,relation],hot}`。**同產業上下游**(focus 內 上中下游)→ 點節點 `imOpenFocus(i)` 塞放大版 `#im-modal`:**上方 = 題材趨勢圖**(`_imRenderSubChart` 重用 `_computeClusterSeries`/`_rebaseSeries`/`_computeIndexSeries`:焦點股加權指數 + 大盤/櫃買 rebase + 三大法人淨流 histogram + 1M/3M/6M/1Y/ALL chips,本地 `_imPeriod`);**下方 = `#im-detail-store` 內 `#imf-{i}` 階層**(axis 上中下游/受惠層 + 個股 + 星級)。**點子產業標題 `imPickSub(i,subIdx)` → 上方圖切到該子產業**(`window.IIA_INDMAP_SUBS[i]={name,all,subs:[{name,tickers}]}`)。為此 **industry-map 純台股 ticker 已併入 `_hist_tickers`** → history.json 的 ticker_close/ticker_net_inst 覆蓋(255/304 有資料,缺的 49 檔 ingest `ticker_close_history` 無 row,該子產業圖顯「暫無足夠歷史」)。modal 滾輪只作用 modal 內(`overscroll-behavior:contain` + wheel guard,`_imBindModalScroll`)、無捲軸(scrollbar 隱藏);關閉 dispose 圖。modal 內個股 → `imShowCross()`(`window.IIA_INDMAP_CROSS`)。graph hover tooltip `placeTip` 夾邊+翻轉防被 `overflow:hidden` 切。**頁面層級「橫跨多焦點明星股」chip 已移除**。純讀,不重算
