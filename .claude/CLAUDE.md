...
- `supabase/functions/db-proxy-public/index.ts` — Edge Function 含 SQL allowlist(目前 **35 條**):
  ...
  - **Q38** 產業地圖(🗺️ 產業地圖 tab,2026-06-07;ingest 週日 09:30 cron 爬 statementdog 焦點產業階層寫 `industry_focus_map`):`SELECT focus_tag, focus_name, axis, axis_kind, axis_order, sub_industry, sub_order, description, ticker, stock_name, market, rating, rating_rank FROM industry_focus_map ORDER BY focus_name, axis_order, sub_order, rating_rank DESC, ticker`。~1095 列編輯型慢變,整表一次撈。**呈現 = 蜘蛛網關聯圖(2026-06-07 改版,原 flat 50-section 清單廢)**:`build_industry_map_page(rows, stocks_info)` —— 節點=50 焦點產業,邊=兩焦點共享個股(`EDGE_MIN_SHARED=2` 候選 + **每節點留最強 `EDGE_TOPK=4` 去 hairball**,462→~136 邊)。**節點發亮 = 該焦點今日成分股的成交值加權平均漲跌幅**(用 ticker join 記憶體 `stocks_info`,紅=今日強/綠=弱/灰空心=今日無成交資料;加權漲幅 ≥ `HOT_THRESHOLD=2.0` 且 cov≥0.2 → 脈動光暈)。**無新 DB query、不碰 allowlist**。`app.js`:`_initIndmapGraph`(手刻 Fruchterman-Reingold 力導向 + radius 解重疊 + 原生 SVG glow filter + viewBox pan/zoom)、`_imHeatColor`(色溫 lerp)、`_imLayout`、tooltip。payload `window.IIA_INDMAP_GRAPH={nodes,edges,hot}`。階層明細改 **點節點 → `imOpenFocus(i)`** 取隱藏 `#im-detail-store` 內 `#imf-{i}` 塞進放大版 `#im-modal`(上中下游/受惠層 + 個股 + 星級)。跨產業關聯:`window.IIA_INDMAP_CROSS` + `imShowCross()`(明星股 chip 仍用,台積電橫跨 24 焦點)。純讀,不重算
