...
- `supabase/functions/db-proxy-public/index.ts` — Edge Function 含 SQL allowlist(目前 **23 條**):
  ...
  - **Q24**(2026-05-27)供「熱門題材」hl_sub cluster header 兩 chip(連續上榜天數 / 近 20 日上榜率)+「📈 趨勢」menu 上圖兩條序列(熱門題材數量 / 題材延續性):`SELECT rank_date, sub_industry FROM theme_history WHERE main_industry='近一年焦點' AND rank_date >= current_date - 180 days`。Python 端 `_cluster_streak_rate20`(merged cluster 用「任一 sub 上榜=cluster 上榜」語意)/ `_build_focus_trend`(對每 T 算過去 N 天內出現過的所有 sub 之平均上榜率)。**注意**:ingest 端寫 main='近一年焦點' 的 theme_history v2 規格自 2026-05-19 起,所以 2026-05-27 當下只有 ~8 天歷史,要等累積到 11 月才到半年。User 想立即看到半年 → 需 ingest 端做 backfill,跨 repo prompt 已交付
