...
- `supabase/functions/db-proxy-public/index.ts` — Edge Function + SQL allowlist(目前 **44 條**,Q1–Q47;每個 Q 的 SQL/語意對照在 ARCHITECTURE.md)。Q43–Q46 已 **slug 參數化**(`where slug = $1`),各策略共用同條;Q47(`select slug from strategy_backtest_public order by slug`)= 策略模擬頁**動態 slug 清單**權威來源。多策略由 generate_html 先跑 Q47 撈全集、再 loop 各帶 slug fetch;新策略 ingest 寫 DB 即自動上架,本 repo 零改動(`STRAT_ORDER` 不再 hardcode,`_STRAT_PREFERRED_ORDER` 僅 Q47 失敗時 fallback、非清單閘)。**tab 顯示序 = 各策略 `payload.metrics.sharpe` 由高到低**(冠軍最左)。slug=`dashboard`(ingest `build_dashboard` 寫的聚合列)從策略 tab 剔除,單獨渲染為**「📊 總儀表板」第一個 sub-tab、預設 active**(`_build_dashboard_html`:① **各策略共識區三欄水平**(明日買進=≥2 策略 watchlist 共選且無任一賣出 / 明日賣出=≥2 策略 sells 同步出場且無任一買進 / 明日分歧=同一檔同時被任一買進+任一賣出;**三類全由本檔從各策略 watchlist+sells 同源推導,不吃 ingest `consensus_picks`**;分歧優先,落入分歧者不再進買賣共識避免跨欄重複)+ 各策略明日買進標的卡格(`consensus_tk`=純買進共識給 🤝、可點開個股 K線/主動ETF modal、雷達★高亮)+ 各策略明日賣出標的卡格(讀 `strategies[].sells`,`when∈{today_exit,next_exit}`;無 sells 整塊隱藏)② 完整 9 指標 1 年回測績效比較表(表頭可排序、預設夏普降序、benchmark 釘底,9 指標 join 各 slug `payload.metrics`);讀 dashboard payload 同走 Q44 slug 參數化,缺則整 tab 隱藏。表頭排序 JS = `app.js dashSortPerf`)。**gap(帶量跳空動能)頁有專屬釐清註記**(`.sim-next-gapclar`/`.sim-bt-pb-clar`):卡片 % = 當日「收盤漲幅」、規則「跳空 ≤+9%」指開盤隔夜跳空,兩者不同數值故收盤可達漲停不矛盾(僅此 slug)。
