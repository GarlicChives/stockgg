...
- `supabase/functions/db-proxy-public/index.ts` — Edge Function 含 SQL allowlist(目前 **23 條**):
  ...
  - **Q18 / Q19 / Q20** 主動式 ETF(ingest `f5faa21` → `edc8d49` v2):Q18 `active_etf_meta` master 按 AUM desc;Q19/Q20 v2 加 has_baseline CTE — DB 內該 ETF 只 1 day holdings 時 lots_chg/action = NULL,UI 顯警示「無前日 baseline」+ chip 不渲(避免硬標 new 失真);Q20 多 pct_of_float(lots × 1000 / stock_meta.shares_outstanding × 100)。**2026-05-27** Q19 SELECT 加 `(SELECT d FROM latest) AS data_date`(latest CTE 的 max holding_date 攤平到每 row),供主動式 ETF 頁每 tab 頂部 bar 與個股 modal「持股主動式 ETF」section 顯示「持股更新 YYYY-MM-DD」。db-proxy(`npm:postgres`)把 DATE 序列化成 `2026-05-27T00:00:00.000Z`,Python 端 `_aetf_date_fmt` 切前 10 碼回標準 YYYY-MM-DD
  - **Q21** 大盤 / 櫃買指數歷史(ingest `11a88d4`,2026-05-28 擴 OHLV 對應 ingest `76f6728`):`market_snapshots` 撈 `^TWII` / `^TWOII` 過去 400 天 daily open/high/low/close/volume,供:(1) cluster chart modal 大盤 / 櫃買 overlay 線(用 close)(2)「📈 趨勢」menu 下圖 ^TWII / ^TWOII 日 K + 成交量(stacked,紅漲綠跌亞洲慣例;OHL 任一 NULL 該日只顯 volume 不畫 K 棒)。ingest 端 backfill 範圍 2024-11-21 ~ today−1(today 過渡期 OHL 暫 NULL,明天起 daily-briefing 用新 code 跑會自動帶 OHL)。2026-05 起 render-time yfinance 全移回 ingest,generate_html.py 不再 `import yfinance`(指數歷史走 Q21、MA20 乖離走 Q13 自算、market_notes ticker 走 Q8)
...
- **新 top nav tab「📈 趨勢」**(2026-05-27,`scripts/generate_html.py` `build_trend_page`):兩張 stacked lightweight-charts。上圖左軸=熱門題材數量、右軸=題材延續性%(雙 priceScale);下圖大盤 ^TWII / 櫃買 ^TWOII 同基期 rebase to 100。`docs/app.js` `_initTrendCharts` 在 showTab('trend') 第一次切過去時 lazy init,payload 走 inline `window.IIA_TREND`。crosshair 雙向 sync + timeScale sync
