...
- `supabase/functions/db-proxy-public/index.ts` — Edge Function 含 SQL allowlist(目前 **33 條**):
  ...
  - **Q31-Q35**(2026-06-03)各頁「資料最後更新時間」badge:回各資料源表最新寫入 timestamptz,stockgg 端 `_fmt_data_stamp` 轉台北時間(UTC+8)顯示 `YYYY/MM/DD HH:MM:SS`。Q31 `trading_rankings` MAX(created_at) market='tw'(熱門題材 / 選股雷達非籌碼,~17:30)、Q32 `ticker_chip_history` MAX(updated_at)(選股雷達>籌碼 sub-tab 額外細標「籌碼資料更新」,21:10)、Q33 `active_etf_holdings` MAX(updated_at)(主動式 ETF,17:35)、Q34 `analysis_reports` MAX(created_at)(市場話題 / 國際金融)、Q35 `market_snapshots` MAX(created_at)(趨勢)。badge 由 `_stamp_badge` 渲染進各 tab pane 頂部右上(`.data-stamp` CSS);單條 query 失敗(403 / 空表)只是該頁不顯 badge,不中斷 render
