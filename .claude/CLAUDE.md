...
- `supabase/functions/db-proxy-public/index.ts` — Edge Function 含 SQL allowlist(目前 **23 條**):
  ...
  - Q13 ticker_close_history 讀取(讓近一年焦點 cluster chart modal 能畫加權指數,因 theme_history 沒此 main 的 row);2026-05-22 起 SELECT 含 `high`(每日盤中最高價)。Q6/Q14/Q15(trading_rankings 今日列)同步加 `high` —— 供「選股雷達 > 新高股」:新高定義 2026-05-22 從「收盤創新高」改為「今日盤中最高價 ≥ 過去 252 日最高盤中價」(收盤定義會把盤中未觸及真實 52 週高、僅收盤超過自家近期收盤上限的股票誤判入選,如 3030;ingest `5a530ea` 持久化 high)。**2026-05-25** 加 `open` / `low`(ingest `4ea7c3e`):Q6/Q14/Q15 加 `open`/`low` 供「早盤漲停股」`open=high=low=close` 判定(用 0.01 容差避 NUMERIC 精度);Q13 加 `open`/`low` 並把 retention 400→**730 天**(2 年),供 P2 個股 modal 日 K 線。早盤漲停股 = focal+sentinel 中 `chg≥9.95 AND |O−C|<0.01 AND |H−C|<0.01 AND |L−C|<0.01`,意義「開盤撮合即鎖、全日無 tick 跌破、收盤仍鎖」;也是交集股 matched 條件之一(rose chip `.fs-mc-elock`)
...
- `docs/kline/<ticker>.json` — 個股 modal 日 K 線 lazy fetch payload(P2 2026-05-25)。**JSON 結構 `{"b": build_stamp_iso, "k": [[d,o,h,l,c,v],...]}`**(原為純 array,後改為 wrapper 物件帶 build_stamp 強迫 hash 每次變化)。單檔 ~14KB(2330),450 檔 ~6MB 總計。**`.gitignore` 排除**(每次 regen 重寫,避免 git repo 膨脹);wrangler v4 部署 Static Assets 不 respect `.gitignore`,kline/ 仍會被上傳到 Cloudflare(實證 wrangler upload count = root 4 + kline 450 = 454 對得上)。相當於用「per-regen 寫靜態檔」取代「client-side db-proxy fetch + 暴露 anon key」。Q13 維持 400 天(730 天會爆 db-proxy 6MB response 上限),所以 kline 也只有 ~400 天,UI 時間粒度 chip 上限 1Y。**`_fetchKline` 必須帶 `?_=Date.now()` cache-bust query 且兼容 `{b,k}` 與舊純 array**(`docs/app.js`):Cloudflare 邊緣節點會 cache 4xx response,deploy 補檔後 path 仍可能 serve 邊緣 cached 404 直到 cache 過期。**為何必須每次 hash 變化**:過去多次 deploy 因 `--log-level` 旗標誤用 abort,wrangler 內部 manifest cache 認為部分 kline blob 已 register,實際 Cloudflare KV 沒收到 → 後續 deploy wrangler 看到「相同內容 hash」就跳過該檔(`/kline/*.json` 但無 `+` / `✨` 標記),fetch 永遠 404。加 build_stamp 後 hash 必新,wrangler 一定 re-upload。(2026-05-25 連環踩過 .gitignore / .assetsignore / wrangler manifest cache 三層誤判)
