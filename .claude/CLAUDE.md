...
- `supabase/functions/db-proxy-public/index.ts` — Edge Function 含 SQL allowlist(目前 **23 條**):
  ...
  - **Q22 / Q23**(2026-05-22)供「選股雷達 > 籌碼股」sub-tab:Q22 `ticker_chip_history` 近 30 天 daily 三大法人分項 net_shares(算近 3 日外資/投信佔量%);Q23 `ticker_holder_dist` 近 60 天 TDCC 集保週資料 —— `big_holder_pct_chg` = 籌碼鎖定率;`levels`(17 級持股分佈 JSON,每級 h/p/s)在 stockgg 端用「級距上限股數 × 股價 < 1000萬」金額定義算散戶持股比,兩週 diff = 散戶持股週變(改金額定義因 TDCC 固定股數級距對高/低價股失真,如 ¥3000 股 1 張即 300萬;兩週用同一最新收盤價,週變不受股價波動干擾)。兩表 ingest 端 `chip_history.py` / `holder_dist.py` 寫入,universe = FOCUS_MAIN。籌碼股版型同其他 sub-tab(無自訂籌碼欄)。**主力 / 前十大券商條件做不到** —— 逐日券商分點是 TWSE 付費資料,無免費來源
